...
p_comp_rmse = plot_comparison(
    ...
    (; neural = data_comp.trajectories.neural,
       constant = data_comp.trajectories.constant);
...
data_emulator = sample_sims(
    spectral_grid,
    (;target = OneBandLongwave(spectral_grid), neural = scheme_neural);
    fac_pert_T = 2f0,
    fac_pert_q = 0f0,
    t_spinup = Day(31),
    sim_time = 180,
...
p_emulator_rmse = plot_comparison(
    ...
    (; neural = data_emulator.trajectories.neural);
